...
BEGIN    
    ...
    IF _type = "empty" THEN
        SELECT DISTINCT ad.appo_id, sa.time, ad.employee_id, e.alias, c.code, ad.service_id, s.name, ca.name, ad.phone_num_id, pn.value, ct.name, CONCAT(p.first_name, ' ', p.last_name), ad.date, ad.start_time, ad.end_time
            FROM saved_appos sa
